        ...
        .then(data => {
            ...
            if (data.length > 0) { // ตรวจสอบว่ามีข้อมูล request หรือไม่
                ...
                if (requestData.status) {
                    document.getElementById('status').innerText = requestData.status;
                    if (requestData.status.toUpperCase() === "PENDING") {
                        document.getElementById('status').style.color = "yellow";
                    }
                    if (requestData.status.toUpperCase() === "REJECTED") {
                        document.getElementById('status').style.color = "#ff0000";
                    }
                    if (requestData.status.toUpperCase() === "APPROVED") {
                        document.getElementById('status').style.color = "#80ff00";
                    }
                }
